...
sub test_discover_policy {
    ...
    is_deeply( $policy, { %test_policy }, 'discover_policy' );
#print Dumper($policy);
};
...
sub test_exists_in_dns {
    ...
    foreach my $dom ( keys %tests ) {
        ...
        ok( $r == $tests{$dom}, "exists_in_dns, $dom" );
    }
};
...
sub test_is_public_suffix {
    ...
    foreach my $dom ( keys %tests ) {
        cmp_ok( $tests{$dom}, '==', $dmarc->is_public_suffix( $dom ), "is_public_suffix: $dom" );
    };
};

sub test_parse_policy {

    my $policy = $dmarc->parse_policy( $test_rec );
    is_deeply( $policy, \%test_policy, "parse_policy" );
#warn Dumper($policy);
};
